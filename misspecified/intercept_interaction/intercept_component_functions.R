...
## betas : coefficients for all covariates
...
sim_linear_int <- function(nA = 75, nB = 75, 
  ...
  while(dat_valid == FALSE){
    ...
    for(i in 1:nA){
      ...
      zA <- betas[1] + betas[3]*mnA + betas[5]*cov1 + betas[6]*cov2 + rand_int
      prA <- expit(zA)
    ...
    for(i in 1:nB){
      ...
      cov1 <- rbinom(n_subj, 1, runif(1, 0.2, 0.5))
      cov2 <- rnorm(n_subj, 0, 1)
      zB <- betas[1] + betas[2] + betas[4]*mnB + betas[5]*cov1 + betas[6]*cov2 + betas[7]*cov1 + rand_int
      prB <- expit(zB)
...
## betas : pre-specified coefficients
...
equiv_linear_inter <- function(studylevel_samples, ipd_samples, studylevel_nocov_samples,
  ...
  vars_studylevel <- all_samps_studylevel[,1:6]
  ...
  vals_ipd <- vals_ipd %>%
    mutate(d1_ipd_female = ((B3)*d0 - B2 - B7)/(B4),
