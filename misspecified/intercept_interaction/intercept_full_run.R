...
full_run_inter <- function(balanced_trials = c(TRUE, FALSE),
    ...
    #Likelihood
      for( i in 1:n)
        ...
        logit(q[i]) <- b[1]+b[2]*drugB[i]+b[3]*drugA_dose[i]+b[4]*drugB_dose[i]+
          b[5]*cov1[i] + b[6]*cov2[i] + b[7]*cov1[i]*drugB[i] + u[trial[i]]
        }
  ...
  ipd_summary <- summary(ipd_fit[,1:8])[[1]]
  ipd_summary_median <- summary(ipd_fit[,1:8])[[2]]
  ...
  names(samps_ipd) <- c("b1", "b2", "b3", "b4", "b5", "b6", "b7")
  ...
  ratios <- c(round(mad_studylevel[,2:7]/mad_ipd[,2:7], 3), NA)
  names(ratios) <- names(mad_ipd)[2:8]
  ratios2 <- c(round(mad_studylevel_nocov[,2:5]/mad_ipd[,2:5], 3), NA, NA, NA)
  names(ratios2) <- names(mad_ipd)[2:8]
  rats <- rbind(ratios, ratios2)
  mad <- as.data.frame(rbind(mad_studylevel[,2:8], mad_studylevel_nocov[,2:8], mad_ipd[,2:8], rats))
