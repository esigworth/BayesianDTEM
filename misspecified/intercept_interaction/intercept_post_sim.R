source("intercept_component_functions.R")
source("intercept_full_run.R")

eval_sim_results_inter <- function(all_sims, tau2, total, betas = c(-.6, -.2, .5, .9, .2, .5, .3), 
  ...
  mad2 <- mad %>%
    pivot_longer(cols = c(b1_mad, b2_mad, b3_mad, b4_mad, b5_mad, b6_mad, b7_mad)) %>%
  ...
  mad2$pos <- c(seq(1.25, 13.25, by = 2), seq(1.75, 13.75, by = 2), seq(1.5, 13.5, by = 2))
  
  
  mad2$param <- rep(c("B1", "B2", "B3", "B4", "B5", "B6", "B7"),3)
  ...
  mad_ratios2$param <- rep(c("B1", "B2", "B3", "B4", "B5", "B6", "B7"), 2)
  mad_ratios2$pos <- c(seq(1.25, 13.25, by = 2), seq(1.75, 13.75, by = 2))
  ...
  for(i in 1:total){
    ...
    eff[[i]] <- ipd[[i]][c(1:6,8),2]^2 / studylevel[[i]][,2]^2
    eff_nocov[[i]] <- ipd[[i]][c(1:4,8),2]^2 / studylevel_nocov[[i]][,2]^2
    
    studylevel_bias[[i]] <- round((studylevel[[i]][,1] - betas_n[c(1:6,8)])/betas_n[c(1:6,8)], 2)
    studylevel_nocov_bias[[i]] <- round((studylevel_nocov[[i]][,1] - betas_n[c(1:4,8)])/betas_n[c(1:4,8)], 2)
    ipd_bias[[i]] <- round((ipd[[i]][,1] - betas_n)/betas_n, 2)
    
    studylevel_se[[i]] <- round((studylevel[[i]][,1] - betas_n[c(1:6,8)])^2, 2)
    studylevel_nocov_se[[i]] <- round((studylevel_nocov[[i]][,1] - betas_n[c(1:4,8)])^2, 2)
  ...
  for(i in 1:total){
    studylevel_beta[i,] <- c(studylevel[[i]][1:6,1], NA)
    studylevel_nocov_beta[i,] <- c(studylevel_nocov[[i]][1:4,1], NA, NA, NA)
    ipd_beta[i,] <- ipd[[i]][1:7,1]
  }
  ...
  studylevel_mse <- c(studylevel_mse[1:6], NA)
  mse <- data.frame(rbind(studylevel_mse, ipd_mse[1:7], studylevel_nocov_mse, mse_ratio, mse_nocov_ratio))
  mse <- data.frame(stat = "mse", type = c("studylevel", "ipd", "studylevel_nocov", "ratio_slcipd", "ratio_slncipd"), mse)
  
  colnames(mse) <- c("stat", "type", "beta1", "beta2", "beta3", "beta4", "beta5", "beta6", "beta7")
  ...
  Type <- rep(c( "studylevel NC", "studylevel", "Full"),7)
  
  
  bias <- data.frame(studylevel = c(studylevel_avg_bias[1:6], NA), 
                     ipd = c(ipd_avg_bias[1:7]), 
                     studylevel_nocov = c(studylevel_nocov_avg_bias[1:4], NA, NA, NA),
                     Beta = c(paste0("B", 1:7, sep = ""))) %>%
    pivot_longer(cols = c(studylevel, studylevel_nocov, ipd),
                 names_to = "Model", values_to = "Percent Bias") 
  
  bias$Type <- Type
  
  err_bars <- data.frame(rbind(studylevel_low_high_bias[1:6,], studylevel_nocov_low_high_bias[1:4,], ipd_low_high_bias[1:7,]))
  names(err_bars) <- c("Low", "High")
  err_bars$Type <- rep(c(rep("studylevel", 6), rep("studylevel NC", 4), rep("ipd", 7)))
  err_bars$Beta <- c(paste0("B", 1:6, sep = ""), paste0("B", 1:4, sep = ""), paste0("B", 1:7, sep = ""))
  ...
  avg_equiv <- data.frame(d0 = seq(-1, 1, by = .2),
                          ...
                          vals_true_female = ((betas[3])*d0 - betas[2] - betas[7])/(betas[4]),
                          lower_ipd_female = NA, 
                          upper_ipd_female = NA, 
  ...
  studylevel_nocov_coverage <- c(studylevel_nocov_coverage[1:4], NA, NA, NA)
  studylevel_coverage <- c(studylevel_coverage, NA)
  
  
  
  coverage <- data.frame(type = c("studylevel", "studylevel_nocov", "ipd"), b1 = NA, b2 = NA, b3 = NA, b4 = NA, b5 = NA, b6 = NA, b7 = NA)
